...
public class MessageProxyFactory {
    static {
        ...
        consoleHandler.setFormatter(new Formatter() { // from class: net.xcharger.sdk.device.MessageProxyFactory.1
            @Override // java.util.logging.Formatter
            public String format(LogRecord record) {
                if (record.getMessage().startsWith("sdk.mqtt")) {
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    String dateFormat = sdf.format(Long.valueOf(record.getMillis()));
    ...
    public static MessageProxy createInstance(MessageProxyOptions ops) {
        ...
        MessageProxy server = new MqttMessageProxy();
        return server;
    }
