...
public class SimpleBase64Encoder {
    ...
    public static String encode(byte[] bytes) {
        ...
        int j = len;
        while (j >= 3) {
            encoded.append(to64(((bytes[i] & 255) << 16) | ((bytes[i + 1] & 255) << 8) | (bytes[i + 2] & 255), 4));
            i += 3;
            j -= 3;
        }
        if (j == 2) {
            encoded.append(to64(((bytes[i] & 255) << 8) | (bytes[i + 1] & 255), 3));
        }
        if (j == 1) {
            encoded.append(to64(bytes[i] & 255, 2));
        }
    ...
    public static byte[] decode(String string) {
        ...
        if (j == 3) {
            ...
            for (int l2 = 1; l2 >= 0; l2--) {
                decoded[k + l2] = (byte) (255 & d2);
                d2 >>= 8;
            }
        }
        if (j == 2) {
            long d3 = from64(encoded, i, 2);
            decoded[k] = (byte) (255 & d3);
        }
        return decoded;
    }
