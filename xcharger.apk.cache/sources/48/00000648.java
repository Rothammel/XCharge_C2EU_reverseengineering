    ...
    protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        ...
        if (decoderState.head == null) {
            ...
            if (head.getBodyLength() == 0) {
                byte start = head.getStartCode();
                byte cmd = head.getCmdCode();
                AnyoMessage message = decodeAnyoMessage(start, cmd, new byte[0]);
                if (message != null) {
        ...
        if (decoderState.head != null && in.remaining() >= (bodyLength = decoderState.head.getBodyLength())) {
            byte[] bodyBytes = new byte[bodyLength];
            in.get(bodyBytes);
            Log.d("AnyoMessageDecoder.doDecode", "receive body: " + FormatUtils.bytesToHexString(bodyBytes));
            byte start2 = decoderState.head.getStartCode();
            byte cmd2 = decoderState.head.getCmdCode();
            AnyoMessage message2 = decodeAnyoMessage(start2, cmd2, bodyBytes);
            if (message2 != null) {
