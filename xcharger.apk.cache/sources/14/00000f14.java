...
public class Draft_6455 extends Draft {
    ...
    public Draft.HandshakeState acceptHandshakeAsServer(ClientHandshake handshakedata) throws InvalidHandshakeException {
        ...
        while (true) {
            ...
            IExtension knownExtension = it2.next();
        ...
        Iterator<IProtocol> it3 = this.knownProtocols.iterator();
        while (true) {
            if (!it3.hasNext()) {
                break;
            }
            IProtocol knownProtocol = it3.next();
    ...
    private ByteBuffer createByteBufferFromFramedata(Framedata framedata) {
        ...
        if ($assertionsDisabled || payloadlengthbytes.length == sizebytes) {
            if (sizebytes == 1) {
                buf.put((byte) ((mask ? AnyoMessage.CMD_RESET_CHARGE : (byte) 0) | payloadlengthbytes[0]));
            } else if (sizebytes == 2) {
    ...
    public int hashCode() {
        ...
        return (result * 31) + (this.protocol != null ? this.protocol.hashCode() : 0);
    }
