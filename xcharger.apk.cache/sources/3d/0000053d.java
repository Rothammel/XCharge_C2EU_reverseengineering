...
public class DCAPProxy {
    ...
    private ConcurrentHashMap<String, SyncConfirm> waitConfirmRequests = null;
    ...
    public void init(Context context) {
        ...
        this.confirmReceiver = new ConfirmReceiver(this, null);
        IntentFilter intent = new IntentFilter(DCAPMessage.ACTION_DCAP_CONFIRM);
        LocalBroadcastManager.getInstance(this.context).registerReceiver(this.confirmReceiver, intent);
    ...
    public void destroy() {
        ...
        while (enumeration.hasMoreElements()) {
            SyncConfirm syncConfirm = enumeration.nextElement();
    ...
    private class ConfirmReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(DCAPMessage.ACTION_DCAP_CONFIRM)) {
                ...
                DCAPMessage confirm = new DCAPMessage().fromJson(body);
                Long seq = null;
                Object data = confirm.getData();
                if (data != null) {
                    String type = confirm.getType();
                    if ("cap".equals(type)) {
                        CAPMessage cap = new CAPMessage().fromJson(JsonBean.ObjectToJson(data));
                        confirm.setData(cap);
                ...
                if (seq != null) {
                    String strSeq = String.valueOf(confirm.getTo()) + seq.longValue();
                    SyncConfirm syncConfirm = (SyncConfirm) DCAPProxy.this.waitConfirmRequests.get(strSeq);
                    if (syncConfirm != null) {
    ...
    public DCAPMessage syncRequest(DCAPMessage request, int timeout) {
        if (this.isInited.get() && sendRequest(request)) {
            ...
            SyncConfirm syncConfirm = new SyncConfirm(this, null);
            this.waitConfirmRequests.put(strSeq, syncConfirm);
            for (int timeout2 = timeout * 1000; syncConfirm.getExitFlag().compareAndSet(false, false) && timeout2 > 0; timeout2 -= 10) {
                try {
                    Thread.sleep(10L);
    ...
    public FeeRate formatFeeRate(ArrayList<ArrayList<Integer>> cardFmt) {
        ...
        for (int i = 0; i < cardFmt.size(); i++) {
            ...
            double powerDouble = new BigDecimal(power / 100.0d).setScale(2, 4).doubleValue();
            double serviceDouble = new BigDecimal(service / 100.0d).setScale(2, 4).doubleValue();
            double delayDouble = new BigDecimal(delay / 100.0d).setScale(2, 4).doubleValue();
    ...
    public boolean checkFeeRateSection(ArrayList<HashMap<String, Object>> priceSections) {
        ...
        while (it2.hasNext()) {
            HashMap<String, Object> section = it2.next();
            String begin = (String) section.get("begin");
            String end = (String) section.get("end");
            sum += calcTimeSection(begin, end);
        }
        if (sum == 86400) {
            sortPriceSections(priceSections);
            return true;
        }
        Log.w("DCAPProxy.checkFeeRateSection", "fee rate not cover 1 day !!! sum: " + sum + ", fee rate: " + JsonBean.getGsonBuilder().create().toJson(priceSections));
        return false;
    }
        ...
        for (int i = 0; i < size - 1; i++) {
            for (int j = i + 1; j < size; j++) {
                if (((String) priceSections.get(i).get("begin")).compareTo((String) priceSections.get(j).get("begin")) > 0) {
                    HashMap<String, Object> tmp = priceSections.get(i);
                    priceSections.set(i, priceSections.get(j));
                    priceSections.set(j, tmp);
                }
            }
        }
    }
    ...
    public void finRequest(FIN_MODE finMode, String userType, String userCode, String chargeId) {
        ...
        DCAPMessage request = createCAPRequest(from, "fin", opt, fin);
        getInstance().sendRequest(request);
    }
    ...
    public void scanAdvertFinishedEvent(String port, String chargeId) {
        ...
        DCAPMessage request = createCAPRequest("guest", "event", opt, event);
        getInstance().sendRequest(request);
    }
    ...
    public Port getPortStatus(String port) {
        String queryId = "device.port." + port;
        String data = queryRequest(5, false, queryId, null);
        if (TextUtils.isEmpty(data)) {
            return null;
        }
        return new Port().fromJson(data);
    }
    ...
    public void updatePortPluginStatus(String port) {
        String queryId = "plugin.update.port." + port;
        queryRequest(0, false, queryId, null);
    }
    ...
    public void networkConnectivityDiagnosis() {
        ...
        if (!TextUtils.isEmpty(gwIp)) {
            boolean pingResult = NetworkUtils.ping2ip(gwIp, 10);
            if (pingResult) {
        ...
        if (!TextUtils.isEmpty(dnsIp)) {
            boolean pingResult2 = NetworkUtils.ping2ip(dnsIp, 10);
            if (pingResult2) {
        ...
        boolean checkResult = NetworkUtils.checkServer("www.qq.com", 80, 10);
        if (checkResult) {
