        ...
        if (userSpecifiedLayers != 0) {
            ...
            while (i <= 32) {
                ...
                if (totalSizeBits <= totalBitsInLayer) {
                    ...
                    if ((!compact || stuffedBits.getSize() <= wordSize * 64) && stuffedBits.getSize() + eccBits <= usableBitsInLayers2) {
                    }
                }
                i++;
            }
            ...
            for (int j = 0; j < rowSize; j++) {
                ...
                for (int k = 0; k < 2; k++) {
                    if (messageBits.get(rowOffset + columnOffset + k)) {
                        matrix.set(alignmentMap[(i4 * 2) + k], alignmentMap[(i4 * 2) + j]);
                    }
                    if (messageBits.get((rowSize * 2) + rowOffset + columnOffset + k)) {
                        matrix.set(alignmentMap[(i4 * 2) + j], alignmentMap[((baseMatrixSize - 1) - (i4 * 2)) - k]);
                    }
                    if (messageBits.get((rowSize * 4) + rowOffset + columnOffset + k)) {
                        matrix.set(alignmentMap[((baseMatrixSize - 1) - (i4 * 2)) - k], alignmentMap[((baseMatrixSize - 1) - (i4 * 2)) - j]);
                    }
                    if (messageBits.get((rowSize * 6) + rowOffset + columnOffset + k)) {
                        matrix.set(alignmentMap[((baseMatrixSize - 1) - (i4 * 2)) - j], alignmentMap[(i4 * 2) + k]);
                    }
                }
            }
            rowOffset += rowSize * 8;
