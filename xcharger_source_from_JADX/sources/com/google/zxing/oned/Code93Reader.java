...
public final class Code93Reader extends OneDReader {
    ...
    private static final int[] CHARACTER_ENCODINGS = {276, 328, 324, 322, 296, 292, 290, 336, 274, 266, 424, 420, 418, 404, 402, 394, 360, 356, 354, StatusLine.HTTP_PERM_REDIRECT, 282, 344, FTPCodes.NEED_ACCOUNT, 326, XCloudProtocolAgent.TIMER_MQTT_CONNECT_BLOCKED, 278, 436, 434, 428, 422, 406, 410, 364, 358, 310, 314, 302, 468, 466, 458, 366, 374, 430, 294, 474, 470, 306, FTPCodes.PENDING_FURTHER_INFORMATION};
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> map) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (nextStart == end || !row.get(nextStart)) {
            ...
            return new Result(decodeExtended(result), (byte[]) null, new ResultPoint[]{new ResultPoint(((float) (start[1] + start[0])) / 2.0f, (float) rowNumber), new ResultPoint(((float) lastStart) + (((float) lastPatternSize) / 2.0f), (float) rowNumber)}, BarcodeFormat.CODE_93);
        }
    }
        ...
        while (i < length) {
            ...
            if (c < 'a' || c > 'd') {
                ...
                switch (c) {
                    ...
                    case 'd':
                        if (next >= 'A' && next <= 'Z') {
                            decodedChar = (char) (next + TokenParser.f168SP);
                            break;
                        } else {
                            throw FormatException.getFormatInstance();
                        }
