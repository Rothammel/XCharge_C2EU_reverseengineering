...
public abstract class UPCEANReader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, int[] startGuardRange, Map<DecodeHintType, ?> hints) throws NotFoundException, ChecksumException, FormatException {
        ...
        if (resultString.length() < 8) {
            ...
            Result decodeResult = new Result(resultString, (byte[]) null, new ResultPoint[]{new ResultPoint(left, (float) rowNumber), new ResultPoint(((float) (endRange[1] + endRange[0])) / 2.0f, (float) rowNumber)}, format);
