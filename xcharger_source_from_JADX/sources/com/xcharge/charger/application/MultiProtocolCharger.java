...
public class MultiProtocolCharger extends Application {
    ...
    private class APPMessageReceiver extends BroadcastReceiver {
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        Log.w("MultiProtocolCharger.onConfigurationChanged", "newConfig: " + JsonBean.getGsonBuilder().create().toJson((Object) newConfig));
        LogUtils.syslog("application com.xcharge.charger onConfigurationChanged: " + JsonBean.getGsonBuilder().create().toJson((Object) newConfig));
        CountrySettingCacheProvider.getInstance().setAppLang(this);
        super.onConfigurationChanged(newConfig);
    }

    private void init() {
        this.receiver = new APPMessageReceiver(this, (APPMessageReceiver) null);
    ...
    private String getProcessName(Context cxt, int pid) {
        List<ActivityManager.RunningAppProcessInfo> runningApps = ((ActivityManager) cxt.getSystemService(UIEventMessage.TYPE_UI_ACTIVITY)).getRunningAppProcesses();
        if (runningApps == null) {
            return null;
        }
    ...
    private void startBootService() {
        try {
            startService(new Intent(this, Class.forName(CLASS_BOOT_SERVICE)));
    ...
    private void stopBootService() {
        try {
            stopService(new Intent(this, Class.forName(CLASS_BOOT_SERVICE)));
        } catch (ClassNotFoundException e) {
    ...
    private void logAllAppsInSystem() {
        try {
            for (ActivityManager.RunningAppProcessInfo info : ((ActivityManager) getSystemService(UIEventMessage.TYPE_UI_ACTIVITY)).getRunningAppProcesses()) {
                String processName2 = info.processName;
                int pid = info.pid;
                LogUtils.syslog("processName: " + processName2 + ",  pid: " + pid + ",  importance: " + info.importance);
            }
        } catch (Exception e) {
