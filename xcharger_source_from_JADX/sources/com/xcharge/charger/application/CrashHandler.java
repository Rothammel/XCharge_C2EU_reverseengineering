...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    public static StringBuffer getTraceInfo(Throwable e) {
        ...
        for (int i = 0; i < stacks.length; i++) {
            ...
            sb.append("class: ").append(stacks[i].getClassName()).append("; method: ").append(stacks[i].getMethodName()).append("; line: ").append(stacks[i].getLineNumber()).append(";  Exception: ").append(String.valueOf(ex.toString()) + StringUtils.f146LF);
        }
    ...
    public static void setError(String e) {
        for (Map.Entry<String, String> m : regexMap.entrySet()) {
            ...
            if (Pattern.compile(m.getKey()).matcher(e).matches()) {
                error = m.getValue();
                return;
            }
        }
