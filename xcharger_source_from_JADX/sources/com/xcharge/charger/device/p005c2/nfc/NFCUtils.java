...
/* renamed from: com.xcharge.charger.device.c2.nfc.NFCUtils */
public class NFCUtils {
    ...
    public static NFCCardIDData distinguishCard(int uuid, String szuuid) {
        ...
        switch ($SWITCH_TABLE$com$xcharge$charger$data$bean$type$CHARGE_PLATFORM()[chargePlatform.ordinal()]) {
            ...
            case 2:
                ...
                } else if (uuid == 1542571595 || uuid == 1543050155) {
                    cardNo = String.valueOf(Long.valueOf(((long) uuid) & XMSZHead.ID_BROADCAST));
    ...
    private static NFCCardIDData recognizeCDDZMCard(int uuid) {
        try {
            ...
            byte[] data = chargerhdNative.chargerhdNFCRead(1, key);
            if (data != null) {
    ...
    private static NFCCardIDData recognizeCDDZUserCard(int uuid) {
        ...
        try {
            String operatorCode = LocalSettingCacheProvider.getInstance().getChargeSetting().getOperatorId();
            ...
            CDDZCardKeySeeds cddzCardKeySeeds = (CDDZCardKeySeeds) new CDDZCardKeySeeds().fromJson(xKeyseed.getSeed());
            ...
            Log.d("NFCUtils.recognizeCDDZUserCard", "AES decrypted: " + FormatUtils.bytesToHexString(echargenet.aesdecrypt(sKey, rawkey, 1)));
            byte[] key = new byte[6];
            ...
            if (data != null) {
                ...
                if (1000 * (((long) EndianUtils.littleBytesToInt(expireTimeBytes)) & XMSZHead.ID_BROADCAST) < System.currentTimeMillis()) {
    ...
    private static NFC_CARD_TYPE getCardType(String cardNo) {
        return NFC_CARD_TYPE.valueOf(cardNo.substring(0, 2));
    }

    private static NFC_CARD_TYPE getPermittedXChargeCardType(String cardNo) {
        NFC_CARD_TYPE cardType = NFC_CARD_TYPE.valueOf(cardNo.substring(0, 2));
    ...
    public static ManageCardData getManageCardInfo(byte[] key, String type) {
        ...
        return (ManageCardData) new ManageCardData().fromJson(reply);
    }
    ...
    public static XSign getUserCardSign(byte[] key) {
        try {
            ...
            return XSign.parse((NFCSign) new NFCSign().fromJson(reply));
        } catch (Exception e) {
    ...
    public static AuthSign getAuthSign(byte[] key) {
        try {
            ...
            return AuthSign.parse((NFCSign) new NFCSign().fromJson(reply));
        } catch (Exception e) {
    ...
    public static NFCSign signU1(byte[] sn, byte[] seed) {
        ...
        System.arraycopy(longToBytes(System.currentTimeMillis() / 1000), 0, data, 0, 4);
    ...
    public static boolean checkU2Sign(byte[] money, byte[] key, XSign sign, byte[] sn) {
        ...
        if (sn != null) {
            CRC32 snCrc32 = new CRC32();
            snCrc32.update(sn);
            if (getU2Sign(money, key, data, longToBytes(snCrc32.getValue())).equals(sign.getSign().toUpperCase())) {
                return true;
            }
        } else if (getU2Sign(money, key, data, (byte[]) null).equals(sign.getSign().toUpperCase())) {
            return true;
        }
        return false;
    }
    ...
    public static boolean consumeU2(int money, int fee, byte[] key, String uuid, String cardNo, int count) {
        if (!setUserCardSign(key, signU2(intToBytes(money - fee), key, 1, (byte[]) null))) {
