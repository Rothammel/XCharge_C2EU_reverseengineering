...
public abstract class AnyoMessage extends JsonBean<AnyoMessage> {
    ...
    public void setHead(AnyoHead head2) {
        this.head = head2;
    }
    ...
    public byte[] toBytes() throws Exception {
        ...
        byte[] bytes = new byte[(body.length + 8)];
    ...
    public byte calcCheckSum(byte rand) throws Exception {
        ...
        System.arraycopy(head2, 0, bytes, 0, head2.length - 1);
        System.arraycopy(body, 0, bytes, head2.length - 1, body.length);
        byte checkSum = rand;
        for (int i = 0; i < len; i++) {
