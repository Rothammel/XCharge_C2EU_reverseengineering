...
public class RemoteStopChargingRequest extends XMSZMessage {
    ...
    public XMSZMessage bodyFromBytes(byte[] bytes) throws Exception {
        if (bytes.length != 4) {
            Log.e("RemoteStopChargingRequest.bodyFromBytes", "body length must be 4 !!! body: " + FormatUtils.bytesToHexString(bytes));
            throw new IllegalArgumentException();
        }
        this.transactionId = ((long) EndianUtils.littleBytesToInt(new byte[]{bytes[0], bytes[1], bytes[2], bytes[3]})) & XMSZHead.ID_BROADCAST;
        return this;
    }
}
