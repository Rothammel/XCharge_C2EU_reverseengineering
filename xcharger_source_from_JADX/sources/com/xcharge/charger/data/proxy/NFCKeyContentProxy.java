...
public class NFCKeyContentProxy {
    ...
    public List<XKeyseed> getAllKeyseed() {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                String seed = DESUtils.decrypt(cursor.getString(cursor.getColumnIndexOrThrow("key")), this.DESKey);
                ...
                XKeyseed xKeyseed = new XKeyseed(groupId, seed);
    ...
    public XKeyseed getKeyseed(String groupId, String type) {
        Cursor cursor = null;
        try {
            Cursor cursor2 = this.resolver.query(this.tbUri, (String[]) null, "group_id=? and nfc_type=?", new String[]{groupId, type}, (String) null);
            ...
            XKeyseed xKeyseed = new XKeyseed(groupId, DESUtils.decrypt(cursor2.getString(cursor2.getColumnIndexOrThrow("key")), this.DESKey));
            xKeyseed.setId(cursor2.getString(cursor2.getColumnIndexOrThrow("_id")));
            xKeyseed.setType(type);
            if (cursor2 == null) {
                return xKeyseed;
            }
    ...
    public boolean saveKeyseed(String groupId, String seed, String type) {
        try {
            ...
            if (xKeyseed != null) {
                if (this.resolver.update(ContentUris.withAppendedId(this.tbUri, Long.parseLong(xKeyseed.getId())), values, (String) null, (String[]) null) > 0) {
                    return true;
                }
    ...
    public boolean clearAllKeyseed() {
        return this.resolver.delete(this.tbUri, (String) null, (String[]) null) > 0;
    }
    ...
    public boolean clearKeyseed(String groupId, String type) {
        ...
        if (!TextUtils.isEmpty(type)) {
            selection = String.valueOf(selection) + " and nfc_type=?";
            selectionArgs = new String[]{groupId, type};
        }
        if (this.resolver.delete(this.tbUri, selection, selectionArgs) > 0) {
            return true;
        }
        return false;
    }
    ...
    public boolean updateKeyseedFromOldVerDb() {
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                XKeyseed xKeyseed = new XKeyseed(cursor.getString(cursor.getColumnIndexOrThrow(ContentDB.NFCKeyTable.GROUP_ID)), DESUtils.decrypt(cursor.getString(cursor.getColumnIndexOrThrow("key")), cipher));
                xKeyseed.setId(cursor.getString(cursor.getColumnIndexOrThrow("_id")));
            ...
            Iterator<XKeyseed> it = keyseedList.iterator();
            while (it.hasNext()) {
                if (!updateKeyseed(it.next())) {
                    return false;
                }
            }
            return true;
