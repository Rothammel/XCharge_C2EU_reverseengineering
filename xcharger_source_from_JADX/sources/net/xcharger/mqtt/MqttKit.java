...
public class MqttKit {
    ...
    public static void publish(String messageName, String version, String data) throws Exception {
        ...
        try {
            ...
            sb.append(messageName).append(StringUtils.SPACE).append(version).append(StringUtils.SPACE).append(id).append(StringUtils.f146LF).append(data);
            logger.log(Level.INFO, "publish topic=  " + topic);
            logger.log(Level.INFO, "publish message " + sb.toString());
            if (Const.binaryMode) {
                msgs.setPayload(encoder.encode(srcId, messageName, version, data));
            } else {
