...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(FTPCodes.COMMAND_PARAMETER_NOT_IMPLEMENTED).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!org.apache.http.HttpHeaders.WARNING.equalsIgnoreCase(fieldName) || !value.startsWith("1")) && (isContentSpecificHeader(fieldName) || !isEndToEnd(fieldName) || networkHeaders.get(fieldName) == null)) {
    ...
    static boolean isEndToEnd(String fieldName) {
        if (org.apache.http.HttpHeaders.CONNECTION.equalsIgnoreCase(fieldName) || "Keep-Alive".equalsIgnoreCase(fieldName) || org.apache.http.HttpHeaders.PROXY_AUTHENTICATE.equalsIgnoreCase(fieldName) || org.apache.http.HttpHeaders.PROXY_AUTHORIZATION.equalsIgnoreCase(fieldName) || org.apache.http.HttpHeaders.f163TE.equalsIgnoreCase(fieldName) || "Trailers".equalsIgnoreCase(fieldName) || org.apache.http.HttpHeaders.TRANSFER_ENCODING.equalsIgnoreCase(fieldName) || org.apache.http.HttpHeaders.UPGRADE.equalsIgnoreCase(fieldName)) {
            return false;
        }
        return true;
    }
