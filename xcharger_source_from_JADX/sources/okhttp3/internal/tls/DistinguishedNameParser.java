...
final class DistinguishedNameParser {
    ...
    /* renamed from: dn */
    private final String f142dn;
    private int end;
    private final int length = this.f142dn.length();
    private int pos;

    DistinguishedNameParser(X500Principal principal) {
        this.f142dn = principal.getName("RFC2253");
    }
    ...
    private String nextAT() {
        ...
        if (this.pos >= this.length) {
            throw new IllegalStateException("Unexpected end of DN: " + this.f142dn);
        }
        this.end = this.pos;
        ...
        while (true) {
            if (this.pos == this.length || this.chars[this.pos] == '+' || this.chars[this.pos] == ',' || this.chars[this.pos] == ';') {
                ...
                if (this.chars[this.pos] >= 'A' && this.chars[this.pos] <= 'F') {
                    ...
                    int i = this.pos;
                    cArr[i] = (char) (cArr[i] + TokenParser.f168SP);
                }
                this.pos++;
            }
        }
        ...
        while (this.pos < this.length) {
            switch (this.chars[this.pos]) {
                case ' ':
                    ...
                    int i = this.end;
                    this.end = i + 1;
                    cArr[i] = TokenParser.f168SP;
                    while (this.pos < this.length && this.chars[this.pos] == ' ') {
                        char[] cArr2 = this.chars;
                        int i2 = this.end;
                        this.end = i2 + 1;
                        cArr2[i2] = TokenParser.f168SP;
                        this.pos++;
                    }
                    if (this.pos != this.length && this.chars[this.pos] != ',' && this.chars[this.pos] != '+' && this.chars[this.pos] != ';') {
                        break;
    ...
    private int getByte(int position) {
        ...
        if (position + 1 >= this.length) {
            throw new IllegalStateException("Malformed DN: " + this.f142dn);
        }
    ...
    public String findMostSpecific(String attributeType) {
        ...
        this.end = 0;
        this.cur = 0;
        this.chars = this.f142dn.toCharArray();
