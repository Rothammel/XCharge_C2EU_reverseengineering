    ...
    private static InetAddress decodeIpv6(String input, int pos, int limit) {
        ...
        while (true) {
            if (i >= limit) {
                ...
                int b2 = b + 1;
                address[b] = (byte) ((value >>> 8) & MotionEventCompat.ACTION_MASK);
                b = b2 + 1;
                address[b2] = (byte) (value & MotionEventCompat.ACTION_MASK);
            }
        }
        if (b != address.length) {
            if (compress == -1) {
    ...
    private static String inet6AddressToAscii(byte[] address) {
        ...
        while (i2 < address.length) {
            if (i2 == longestRunOffset) {
                ...
                result.writeHexadecimalUnsignedLong((long) (((address[i2] & 255) << 8) | (address[i2 + 1] & MotionEventCompat.ACTION_MASK)));
                i2 += 2;
            }
        }
