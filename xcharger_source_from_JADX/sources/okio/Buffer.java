...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    private static final byte[] DIGITS = {AnyoMessage.CMD_SYNC_TIME, AnyoMessage.CMD_RESET_SYSTEM, 50, 51, 52, AnyoMessage.CMD_BIND_USER, AnyoMessage.CMD_QUERY_SYS_INFO, AnyoMessage.CMD_UPDATE_SYS_INFO, AnyoMessage.CMD_QUERY_FEE_POLICY, AnyoMessage.CMD_UPDATE_FEE_POLICY, 97, 98, 99, 100, 101, 102};
    ...
    public short readShort() {
        ...
        int limit = segment.limit;
        if (limit - pos < 2) {
            return (short) (((readByte() & 255) << 8) | (readByte() & MotionEventCompat.ACTION_MASK));
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
        int pos3 = pos2 + 1;
        int s = ((data[pos] & 255) << 8) | (data[pos2] & MotionEventCompat.ACTION_MASK);
        this.size -= 2;
        if (pos3 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    public int readInt() {
        ...
        int limit = segment.limit;
        if (limit - pos < 4) {
            return ((readByte() & 255) << AnyoMessage.CMD_REPORT_CHARGE_STOPPED) | ((readByte() & 255) << 16) | ((readByte() & 255) << 8) | (readByte() & 255);
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
        int pos3 = pos2 + 1;
        int pos4 = pos3 + 1;
        int pos5 = pos4 + 1;
        byte b = ((data[pos] & 255) << AnyoMessage.CMD_REPORT_CHARGE_STOPPED) | ((data[pos2] & 255) << 16) | ((data[pos3] & 255) << 8) | (data[pos4] & 255);
        this.size -= 4;
        if (pos5 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    public long readLong() {
        ...
        int limit = segment.limit;
        if (limit - pos < 8) {
            return ((((long) readInt()) & XMSZHead.ID_BROADCAST) << 32) | (((long) readInt()) & XMSZHead.ID_BROADCAST);
        }
        byte[] data = segment.data;
        int pos2 = pos + 1;
        int pos3 = pos2 + 1;
    ...
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public int selectPrefix(okio.Options r23, boolean r24) {
        /*
            r22 = this;
            r0 = r22
        ...
        L_0x0009:
            return r10
        L_0x000a:
            okio.ByteString r21 = okio.ByteString.EMPTY
            r0 = r23
            r1 = r21
    ...
    public Buffer writeShort(int s) {
        ...
        int limit2 = limit + 1;
        data[limit] = (byte) ((s >>> 8) & MotionEventCompat.ACTION_MASK);
        data[limit2] = (byte) (s & MotionEventCompat.ACTION_MASK);
        tail.limit = limit2 + 1;
        this.size += 2;
        return this;
    }
    ...
    public Buffer writeInt(int i) {
        ...
        int limit = tail.limit;
        int limit2 = limit + 1;
        data[limit] = (byte) ((i >>> 24) & MotionEventCompat.ACTION_MASK);
        int limit3 = limit2 + 1;
        data[limit2] = (byte) ((i >>> 16) & MotionEventCompat.ACTION_MASK);
        int limit4 = limit3 + 1;
        data[limit3] = (byte) ((i >>> 8) & MotionEventCompat.ACTION_MASK);
        data[limit4] = (byte) (i & MotionEventCompat.ACTION_MASK);
        tail.limit = limit4 + 1;
        this.size += 4;
        return this;
    }
    ...
    private ByteString digest(String algorithm) {
        try {
            ...
            return ByteString.m41of(messageDigest.digest());
    ...
    private ByteString hmac(String algorithm, ByteString key) {
        try {
            ...
            return ByteString.m41of(mac.doFinal());
        } catch (NoSuchAlgorithmException e) {
            throw new AssertionError();
