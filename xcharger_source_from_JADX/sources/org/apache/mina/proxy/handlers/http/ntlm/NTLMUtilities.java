...
public class NTLMUtilities implements NTLMConstants {
    ...
    public static final byte[] getOsVersion() {
        ...
        try {
            ...
            try {
                ...
                writeOSVersion(Byte.parseByte(version.substring(0, 1)), Byte.parseByte(version.substring(2, 3)), 0, osVer, 0);
                return osVer;
            } catch (Exception e2) {
                return DEFAULT_OS_VERSION;
            }
    ...
    public static final byte[] readSecurityBufferTarget(byte[] msg, int securityBufferOffset) {
        ...
        System.arraycopy(msg, ByteUtilities.makeIntFromByte4(securityBuffer, 4), secBufValue, 0, length);
        return secBufValue;
    }
    ...
    public static final String extractTargetNameFromType2Message(byte[] msg, Integer msgFlags) throws UnsupportedEncodingException {
        ...
        if (ByteUtilities.isFlagSet(msgFlags == null ? extractFlagsFromType2Message(msg) : msgFlags.intValue(), 1)) {
    ...
    public static final byte[] extractTargetInfoFromType2Message(byte[] msg, Integer msgFlags) {
        if (!ByteUtilities.isFlagSet(msgFlags == null ? extractFlagsFromType2Message(msg) : msgFlags.intValue(), 8388608)) {
            return null;
        }
    ...
    public static final byte[] createType3Message(String user, String password, byte[] challenge, String target, String workstation, Integer serverFlags, byte[] osVersion) {
        if (challenge == null || challenge.length != 8) {
        ...
        } else if (osVersion == null || osVersion.length == 8) {
            ...
            try {
                ...
                writeSecurityBufferAndUpdatePointer(baos, (short) dataNTLMResponse.length, writeSecurityBufferAndUpdatePointer(baos, (short) dataLMResponse.length, targetName.length + pos + userName.length + workstationName.length));
