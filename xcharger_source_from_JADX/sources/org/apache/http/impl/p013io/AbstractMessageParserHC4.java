...
@NotThreadSafe
/* renamed from: org.apache.http.impl.io.AbstractMessageParserHC4 */
public abstract class AbstractMessageParserHC4<T extends HttpMessage> implements HttpMessageParser {
    ...
    public AbstractMessageParserHC4(SessionInputBuffer buffer, LineParser lineParser2, MessageConstraints constraints) {
            ...
            if (inbuffer.readLine(current) == -1 || current.length() < 1) {
                ...
                if ((current.charAt(0) == ' ' || current.charAt(0) == 9) && previous != null) {
                    ...
                    if (maxLineLen <= 0 || ((previous.length() + 1) + current.length()) - i2 <= maxLineLen) {
                        previous.append(TokenParser.f168SP);
