...
public class ProtocolExec implements ClientExecChain {
        ...
        try {
            ...
            if (uri2 != null) {
                if (route.getProxyHost() == null || route.isTunnelled()) {
                    ...
                    uri = URIUtilsHC4.rewriteURI(uri2, route.getTargetHost(), true);
                } else {
                    uri = URIUtilsHC4.rewriteURI(uri2);
                }
                request.setURI(uri);
    ...
    public CloseableHttpResponse execute(HttpRoute route, HttpRequestWrapper request, HttpClientContext context, HttpExecutionAware execAware) throws IOException, HttpException {
        ...
        if (original instanceof HttpUriRequest) {
            ...
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "Unable to parse '" + uriString + "' as a valid URI; " + "request URI and Host header may be inconsistent", ex);
                }
            }
        }
        request.setURI(uri);
        rewriteRequestURI(request, route);
        HttpHost virtualHost = (HttpHost) request.getParams().getParameter("http.virtual-host");
        ...
        if (!(uri == null || (userinfo = uri.getUserInfo()) == null)) {
            ...
            credsProvider.setCredentials(new AuthScope(target.getHostName(), target.getPort()), new UsernamePasswordCredentials(userinfo));
        }
