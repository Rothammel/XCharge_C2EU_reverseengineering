...
public class FinderPatternFinder {
    ...
    public static boolean foundPatternCross(int[] stateCount) {
        ...
        if (totalModuleSize >= 7) {
            ...
            return (Math.abs(moduleSize - stateCount[0]) >= maxVariance || Math.abs(moduleSize - stateCount[1]) >= maxVariance || Math.abs((3.0f * moduleSize) - stateCount[2]) >= 3.0f * maxVariance || Math.abs(moduleSize - stateCount[3]) >= maxVariance || Math.abs(moduleSize - stateCount[4]) >= maxVariance) ? false : false;
        }
        return false;
    }
