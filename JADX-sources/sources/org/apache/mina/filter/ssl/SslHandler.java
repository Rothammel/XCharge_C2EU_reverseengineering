...
public class SslHandler {
    ...
    public WriteFuture writeNetBuffer(IoFilter.NextFilter nextFilter) throws SSLException {
        ...
        try {
            this.sslFilter.filterWrite(nextFilter, this.session, new DefaultWriteRequest(writeBuffer, defaultWriteFuture));
            while (needToCompleteHandshake()) {
                try {
                    ...
                    if (outNetBuffer == null || !outNetBuffer.hasRemaining()) {
                        ...
                        defaultWriteFuture2 = new DefaultWriteFuture(this.session);
                        this.sslFilter.filterWrite(nextFilter, this.session, new DefaultWriteRequest(outNetBuffer, defaultWriteFuture2));
                    }
                    defaultWriteFuture = defaultWriteFuture2;
